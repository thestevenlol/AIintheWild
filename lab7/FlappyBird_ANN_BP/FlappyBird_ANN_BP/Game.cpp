...
{
//	m_window.setKeyRepeatEnabled(false); // This only works if you use the Event processing through the window handler.
										// If using isKeyPressed() then this makes no difference since you are directly querying the keyboard state.
...
void Game::update(float dt)
{
	...
	if (capture)	// Capture training data before we update everything.
					// Otherwise the players decision to jump or not jump will be recorded against the wrong data !! Makes for bad training data :(
	{
		string tempString = "";
