...
public:
    ...
    MoveAway(Grid& g) : Routine()    // Superclass constructor
    {
        this->destX = 1;
        this->destY = 1;
    ...
    {
        return (abs(droid->x - enemy->x) <= droid->range
            && abs(droid->y - enemy->y) <= droid->range);       // Checking distance in terms of Grid squares not pixels.
    }
