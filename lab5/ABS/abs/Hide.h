...
//
// This behaviour picks 2 other droids at random A and B.
// It then tries to position itself so that B is located between it and A.
// In effect it hides from A behind B.
//
// This probably needs to be refactored into a sequence of "Find Hide Location" followed by a "MoveTo" behaviour.
//
class Hide : public Routine
{
...
public:
    ...
    Hide(int droidA, int droidB, Grid& g) : Routine()    // Superclass constructor
    {
        this->destX = 1;
        this->destY = 1;
        this->target = g.getGridLocation(destX, destY);
        this->routineType = "Hide";
        this->routineGrid = &g;
    ...
    void act(Droid* droid, Grid& grid)
    {
        ...
        if (isRunning())
        {
            ...
            // Now we need to figure out where the hide position is.
            sf::Vector2f AB = sf::Vector2f(droidBx-droidAx, droidBy-droidAy);
            float lenAB = grid.length(AB);
            sf::Vector2f ABn = droid->normalize(AB);
            sf::Vector2f C = sf::Vector2f(droidBx + ABn.x * distanceToHidePoint, droidBy + ABn.y * distanceToHidePoint); // C is the porposed hide point
