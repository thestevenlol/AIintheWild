...
// This version is a copy from the Google Tiny Sorter Example: https://experiments.withgoogle.com/tiny-sorter/view
// However their version was processing the camera frames too quickly.
// I added a timeout in the gotResult function so it would give the Arduino enough time to swing the arm back into position 
// before the app processes the next frame.
...
//    console.log(results);
    ...
    if (class1[0].confidence > 0.90) {
      ...
      try {
        if (window.port) {
          console.log("Sending command 1 (left) to Arduino");
          window.port.send(view);
          shouldFreezeFrame = true;
          splashLeft.trigger();
          isLeftPic = false;
        } else {
          console.warn("Arduino not connected - please click CONNECT ARDUINO first");
        }
      } catch (e) {
        console.error("Failed to send command to Arduino:", e);
      }
    } else if (class2[0].confidence > 0.90) {
      view[0] = 2;
      try {
        if (window.port) {
          console.log("Sending command 2 (right) to Arduino");
          window.port.send(view);
          shouldFreezeFrame = true;
          splashRight.trigger();
          isLeftPic = true;
...
//  let view = new Uint8Array(1);
//  console.log(port.receive(view));
  label = results[0].label;
  // Classifiy again!
  setTimeout(() => {	// OC Edit. Added a time out because the app was continuing on to the next frame 
      classifyVideo()	// before the Arduino was finished doing its action.
    }, 4000);  			// The effect was that the Arduino had not swung the arm back up to present the camera with the 
}						// blank background for it to recognise an empty picture. The camera was therefore picking up background
						// images that it was not trained on and then inferring an incorrect class.
