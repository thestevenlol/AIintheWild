...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    queue.push((problem.getStartState(), [], 0), 0 + heuristic(problem.getStartState(), problem)) # f(n) = g(n) + h(n), initial g(n) = 0
    print("Start queue:", queue.heap)
    
    visited = set()

    while not queue.isEmpty():
        currentState, steps, current_cost = queue.pop()
        ...
        for state, action, cost in problem.getSuccessors(currentState):
            ...
            new_priority = new_cost + heuristic(state, problem)
            queue.push((state, steps + [ action ], new_cost), new_priority)
